...
class LightGBM:
    ...
    def train(self, X_train, y_train, X_test, y_test, init_model=None):
        train_data = lgb.Dataset(X_train, label=y_train)
        test_data = lgb.Dataset(X_test, label=y_test)
        if init_model is not None:
            self.model = lgb.train(self.params, train_data, 100, valid_sets=[test_data], init_model=init_model)
        else:
