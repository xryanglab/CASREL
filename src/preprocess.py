...
def categorize_splice_prob(prob, low_threshold=0.4, high_threshold=0.6):
    """
    Categorize a splice probability into integer labels.

    Categories:
      NaN            -> 0  (handled via na_action='ignore', fillna(0))
      < low_threshold  -> 1
      low_threshold <= prob <= high_threshold -> 2
      > high_threshold -> 3

    Parameters
    ----------
    prob : float
        The splice probability value.
    low_threshold : float
        Lower boundary between category 1 and 2 (default: 0.4).
    high_threshold : float
        Lower boundary between category 2 and 3 (default: 0.6).
    """
...
def main():
    ...
    parser.add_argument(
        ...
        default=0.4,
        help=(
            "Lower probability threshold for splice category boundaries. "
            "Prob < low_threshold -> 1, low_threshold <= prob <= high_threshold -> 2, "
            "prob > high_threshold -> 3. (default: 0.4)"
        )
    )
    parser.add_argument(
        "--high_threshold",
        ...
        default=0.6,
        help=(
            "Upper probability threshold for splice category boundaries. "
            "Prob < low_threshold -> 1, low_threshold <= prob <= high_threshold -> 2, "
            "prob > high_threshold -> 3. (default: 0.6)"
        )
    )
    ...
    if args.low_threshold >= args.high_threshold:
        parser.error(
            f"--low_threshold ({args.low_threshold}) must be strictly less than "
            f"--high_threshold ({args.high_threshold})."
        )

    print(
        f"Using splice probability thresholds: "
        f"< {args.low_threshold} -> 1, "
        f"{args.low_threshold} ~ {args.high_threshold} -> 2, "
        f"> {args.high_threshold} -> 3, "
        f"NaN -> 0"
    )
