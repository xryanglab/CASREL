...
### The experimental procedure is the same as that for GRSF1 and HNRNPK.

net_files <- c(
  "E:/rasnet/revision/nsclc/Regulator_contribution_summary_knee.csv",
  "E:/rasnet/revision/crc/Regulator_contribution_summary_knee.csv",
  "E:/rasnet/revision/hcc2/Regulator_contribution_summary_knee.csv"
)
...
preprocess_net <- function(net_file) {
  net   <- fread(net_file, data.table = FALSE, colClasses = "character")
  no_chr <- !grepl("^chr", net$AS_Site)
  if (any(no_chr)) net$AS_Site[no_chr] <- paste0("chr", net$AS_Site[no_chr])
  net
}
...
avail_k      <- k562_obj$rbp_list
avail_h      <- hepg2_obj$rbp_list
available_rbps <- intersect(avail_k, avail_h)
other_rbps   <- setdiff(available_rbps, target_rbp)
set.seed(123)
selected_rbps <- sample(other_rbps, min(100, length(other_rbps)))

compute_rbps <- c(target_rbp, selected_rbps)
compute_rbps <- intersect(compute_rbps, available_rbps)
...
get_das <- function(rbp) {
  if (exists(rbp, envir = rbp_cache, inherits = FALSE))
    get(rbp, envir = rbp_cache, inherits = FALSE)
  else NULL
}

get_sig_sites <- function(diff_df, cell_filter, net_keys) {
  sub <- diff_df %>% filter(cell == cell_filter)
  sub$key <- paste(sub$rbp, sub$site, sep = "_")
  if (use_fdr) {
    sub <- sub %>% filter((fdr < pval_threshold | abs(das) > das_threshold) & key %in% net_keys)
  } else {
    sub <- sub %>% filter((pvalue < pval_threshold | abs(das) > das_threshold) & key %in% net_keys)
  }
  unique(sub$site)
}
...
for (net_file in net_files) {
  net_name <- basename(dirname(net_file))
  cat("\nProcessing net:", net_name, "\n")
  
  net <- net_list[[net_file]]
  site_parts <- strsplit(net$AS_Site, "_")
  net$site   <- sapply(site_parts, function(x) paste(x[1:3], collapse = "_"))
  
  target_sites <- unique(net$site[net$RBP == target_rbp])
  target_sites <- intersect(target_sites, covered_sites)
  n_target     <- length(target_sites)
  cat("  Target RBP sites (after filtering):", n_target, "\n")
  if (n_target == 0) next
  ...
  for (rbp in selected_rbps) {
    ...
    rbp_net_keys <- paste(rbp, net$site, sep = "_")
    
    sig_k562  <- get_sig_sites(diff_df, "K562",  rbp_net_keys)
    rate_k562 <- length(intersect(sig_k562,  target_sites)) / n_target
    
    sig_hepg2  <- get_sig_sites(diff_df, "HEPG2", rbp_net_keys)
    rate_hepg2 <- length(intersect(sig_hepg2, target_sites)) / n_target
    
    result_list[[length(result_list) + 1]] <- data.frame(
      RBP = rbp, ValidationRate = rate_k562,  Cell = "K562",  Dataset = net_name,
      stringsAsFactors = FALSE
    )
    result_list[[length(result_list) + 1]] <- data.frame(
      RBP = rbp, ValidationRate = rate_hepg2, Cell = "HEPG2", Dataset = net_name,
      stringsAsFactors = FALSE
    )
  }
}
...
fwrite(final_results, paste0(target_rbp, "_cross_validation_other_rbps.csv"), row.names = FALSE)

target_das <- get_das(target_rbp)
ref_line_list <- list()
...
for (net_file in net_files) {
  ...
  site_parts   <- strsplit(net$AS_Site, "_")
  net$site     <- sapply(site_parts, function(x) paste(x[1:3], collapse = "_"))
  target_sites <- intersect(unique(net$site[net$RBP == target_rbp]), covered_sites)
  if (length(target_sites) == 0 || is.null(target_das)) next
  
  rbp_net_keys <- paste(target_rbp, net$site, sep = "_")
  
  for (cl in c("K562", "HEPG2")) {
    sig   <- get_sig_sites(target_das, cl, rbp_net_keys)
    rate  <- length(intersect(sig, target_sites)) / length(target_sites)
    ref_line_list[[length(ref_line_list) + 1]] <- data.frame(
      Cell = cl, Dataset = net_name, ref_rate = rate, stringsAsFactors = FALSE
    )
  }
}
ref_line <- do.call(rbind, ref_line_list)

p <- ggplot(final_results, aes(x = Dataset, y = ValidationRate, fill = Cell)) +
  geom_violin(position = position_dodge(width = 0.9), scale = "width", trim = FALSE) +
  geom_jitter(position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.9),
              size = 1.2, alpha = 0.6) +
  geom_hline(data = ref_line, aes(yintercept = ref_rate),
             linetype = "dashed", color = "grey40", linewidth = 0.8) +
  facet_wrap(~ Cell, scales = "free_y") +
  labs(
    title = paste0("Validation rates of random RBPs on ", target_rbp, " target sites"),
    y = "Validation Rate", x = "Dataset"
  ) +
...
print(p)
ggsave("cross_validation_violin.pdf", p, width = 8, height = 5)
...
truth <- read.table('human_binding_gene.csv', sep = ',', header = T, stringsAsFactors = FALSE)
truth[1:5,]
